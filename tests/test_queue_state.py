...
class TestHeartbeatTimeout:
    ...
    def test_pending_delivery_not_timed_out(self) -> None:
        """A claimed task pending delivery must not be timed out,
        even if the heartbeat timestamp is stale (slow flush)."""
        state = _make_state()
        state.push("t1", {}, now=100.0, idempotency_key=_key())
        state.claim(now=200.0, idempotency_key=_key())
        # No confirm_delivery — simulates claim still being flushed
    ...
    def test_requeue_clears_pending_delivery(self) -> None:
        """When a delivered task is re-queued via heartbeat timeout,
        its old task_id must be removed from _pending_delivery so
        ...
        assert "t1" not in state._pending_delivery
