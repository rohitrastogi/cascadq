...
from cascadq.models import QueueMeta, Snapshot, TaskStatus
...
def _make_state(
    ...
    qf = Snapshot(
        metadata=QueueMeta(created_at=created_at, payload_schema=schema),
        next_sequence=0,
        tasks=[],
    )
    return TaskQueue(name=name, queue_file=qf, version=1)


class TestFIFOOrdering:
    def test_claims_return_tasks_in_push_order(self) -> None:
        state = _make_state()
        state.push("t1", {"x": 1}, now=100.0, push_key=_key())
        state.push("t2", {"x": 2}, now=101.0, push_key=_key())
        state.push("t3", {"x": 3}, now=102.0, push_key=_key())

        task1 = state.claim(now=200.0, claim_key=_key())
        task2 = state.claim(now=200.0, claim_key=_key())
        task3 = state.claim(now=200.0, claim_key=_key())
...
class TestClaimFinishLifecycle:
    def test_claim_then_finish(self) -> None:
        state = _make_state()
        state.push("t1", {}, now=100.0, push_key=_key())
        task = state.claim(now=200.0, claim_key=_key())
    ...
    def test_claim_empty_queue_raises(self) -> None:
        ...
        with pytest.raises(QueueEmptyError):
            state.claim(now=200.0, claim_key=_key())
    ...
    def test_finish_after_compaction_is_idempotent(self) -> None:
        ...
        # t1 is gone, but sequence 0 <= compaction_watermark (0)
        state.finish("t1", sequence=0)
...
class TestClaimIdempotency:
    def test_same_key_returns_same_task(self) -> None:
        state = _make_state()
        state.push("t1", {}, now=100.0, push_key=_key())
        result1 = state.claim(now=200.0, claim_key="k1")
        result2 = state.claim(now=200.0, claim_key="k1")
    ...
    def test_replay_does_not_mutate(self) -> None:
        ...
        state = _make_state()
        state.push("t1", {}, now=100.0, push_key=_key())
        first = state.claim(now=200.0, claim_key="k1")
        assert first.mutated
        replay = state.claim(now=201.0, claim_key="k1")
        assert not replay.mutated

    def test_different_keys_claim_different_tasks(self) -> None:
        state = _make_state()
        state.push("t1", {}, now=100.0, push_key=_key())
        state.push("t2", {}, now=101.0, push_key=_key())
        task1 = state.claim(now=200.0, claim_key="k1")
        task2 = state.claim(now=200.0, claim_key="k2")
    ...
    def test_replay_after_requeue_claims_new_task(self) -> None:
        ...
        task_retry = state.claim(now=200.0, claim_key="k1")
...
class TestHeartbeatTimeout:
    ...
    def test_pending_activation_not_timed_out(self) -> None:
        """A claimed task pending activation must not be timed out,
        even if the heartbeat timestamp is stale (slow flush)."""
        state = _make_state()
        state.push("t1", {}, now=100.0, push_key=_key())
        state.claim(now=200.0, claim_key=_key())
        # No activate_lease — simulates claim still being flushed
    ...
    def test_requeue_clears_pending_activation(self) -> None:
        """When a task is re-queued via heartbeat timeout,
        its old task_id must be removed from _pending_activation so
        the set doesn't leak."""
        state = _make_state()
        state.push("t1", {}, now=100.0, push_key=_key())
        state.claim(now=200.0, claim_key=_key())
        state.activate_lease("t1")
        ...
        assert "t1" not in state._pending_activation
...
class TestCompaction:
    ...
    def test_push_key_ttl_cleanup(self) -> None:
        """Push keys older than the TTL are removed during compaction."""
        state = _make_state()
        state.push("t1", {}, now=100.0, push_key="key1")
        # Compact well past the 300s TTL
        state.compact(now=500.0)
        # Key should be expired
        assert "key1" not in state.snapshot().push_keys
...
class TestPayloadValidation:
    def test_invalid_payload_raises(self) -> None:
        ...
        with pytest.raises(PayloadValidationError):
            state.push("t1", {"not_url": 123}, now=100.0, push_key=_key())
    ...
    def test_valid_payload_accepted(self) -> None:
        ...
        state.push(
            "t1", {"url": "http://example.com"}, now=100.0, push_key=_key(),
        )
        snapshot = state.snapshot()
        assert len(snapshot.tasks) == 1
...
class TestSnapshot:
    def test_snapshot_includes_all_tasks(self) -> None:
        state = _make_state()
        state.push("t1", {}, now=100.0, push_key=_key())
        state.push("t2", {}, now=101.0, push_key=_key())
        state.push("t3", {}, now=102.0, push_key=_key())
        state.claim(now=200.0, claim_key=_key())
        state.activate_lease("t1")
