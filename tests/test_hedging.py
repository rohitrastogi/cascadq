...
class TestHedgedWrite:
    ...
    async def test_conflict_plus_transport_error_raises_transport_error(
        ...
        async def conflict_then_error(
            ...
            call_count += 1
            if call_count == 1:
                # Primary: sleep, then lose the race to the hedge and
                # finish first with a self-conflict.
                await asyncio.sleep(0.1)
                return await store.write(key, data, version)
            # Hedge: write successfully, then fail later with a transport
            # error so the conflict path awaits this exception.
